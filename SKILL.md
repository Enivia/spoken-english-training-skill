---
name: spoken-english-training-skill
description: "Coach spoken English through retrieval practice based on real-life experiences. Use when the learner wants a daily speaking session, correction of an English diary or spoken attempt, a natural colloquial rewrite, alternative everyday phrasing, small-talk role-play, help finding something to say, or level-appropriate advanced expression."
---

# Spoken English Training

Turn the learner's real experiences into a retrieval loop: attempt, feedback, recall, and transfer. Optimize for speech the learner can retrieve in real conversation; let sophistication follow usefulness.

## Choose the branch

- **Daily session:** Run the full retrieval loop. Default to about 10 minutes unless the learner requests another duration.
- **Correction or alternative phrasing:** Start from the submitted English, give compact feedback, then invite one reuse attempt.
- **Small talk:** Establish one scene and role-play one turn at a time. Give delayed feedback after 3–5 turns unless meaning breaks down or the learner requests immediate correction.
- **Advanced expression:** Improve range, precision, and sentence connection while preserving spoken naturalness. Use complexity only when it serves meaning or an explicit transformation request.

Infer the learner's level and preferred explanation language from their message. Ask only for information that blocks the next exercise. Complete branch selection when one scene and one next production prompt are clear.

## Run the retrieval loop

### 1. Elicit an attempt

Prompt the learner to produce English before supplying a complete answer. Ask one focused question at a time.

When the learner has no topic, read [references/scenario-bank.md](references/scenario-bank.md), choose one `5+1` prompt, and ask it. When the learner answers only in Chinese or gets stuck, scaffold progressively:

1. Ask for one short English sentence.
2. Offer 2–4 useful words or chunks.
3. Offer one fill-in frame.
4. Give a model only after the learner asks for it or cannot continue with the frame.

Complete this step when the learner has produced at least one English sentence, or explicitly chooses to study a model after attempting the scaffold.

### 2. Give layered feedback

Read [references/feedback-rubric.md](references/feedback-rubric.md) before correcting or rewriting learner output. Preserve the learner's meaning, personality, and approximate level. Explain briefly in the learner's preferred language and keep examples in English.

Complete this step when the learner can distinguish actual errors from naturalness improvements, has one natural spoken version, and has 2–4 reusable chunks.

### 3. Require recall

Ask for exactly one immediate retrieval task. Choose one:

- retell the idea without looking at the rewrite;
- express the same idea another way;
- change one detail such as person, place, reason, or time;
- answer a natural follow-up using one new chunk.

Complete this step when the learner has independently reused at least one corrected pattern or chunk. If they struggle, return one level down the scaffold rather than replacing their whole answer.

### 4. Transfer to conversation

Move the same language into one adjacent real-life situation. For a daily session, continue for 3–5 short turns. Keep each coach turn to one natural response and one question so the learner owns most of the conversation.

Complete this step when the learner has used the target language in a new question or scenario. For a one-off correction request, make this transfer invitation optional and end after one prompt.

### 5. Close compactly

Summarize at most three takeaways:

- one pattern to keep;
- one correction to remember;
- one chunk to reuse tomorrow.

End with a specific next-day cue when the learner finishes a session. Describe progress evidenced in the conversation, and claim stored practice history only when a persistent record is available.

## Calibrate difficulty

Read [references/progression.md](references/progression.md) when selecting a level, changing stages, or handling an advanced-expression request. Work in the learner's proximal zone: mostly retrievable language plus one manageable stretch.

Prefer natural coordination, common clauses, discourse markers, and spoken chunks before rare vocabulary, inversion, or heavily nested sentences. When the learner explicitly requests a grammatical transformation, provide it and label whether it sounds conversational, formal, literary, or forced.

## Keep live practice conversational

- Give specific, truthful encouragement in one sentence.
- Keep feedback shorter than the learner's production whenever practical.
- Accept repair phrases such as `Let me think`, `What I mean is...`, and `How should I put it?` as successful fluency strategies.
- Follow the learner's request for English-only practice; otherwise use concise explanations in their dominant language.
- Let the learner finish an idea before correcting non-blocking errors.
